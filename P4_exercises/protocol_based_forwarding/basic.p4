...
const bit<32> h1 = 0x0a000101;
const bit<32> h2 = 0x0a000202;
const bit<32> h3 = 0x0a000303;
/*************************************************************************
*********************** H E A D E R S  ***********************************
*************************************************************************/
...
/* select statement used for branching*/
      transition select(hdr.ethernet.etherType){
        TYPE_IPV4 : parse_ipv4;
        default : accept;
      }
    }
/*transitioned to parse ipv4 header*/
    state parse_ipv4 {
...
/* table for dropping icmp packets when packets are to be transferred between h1 and h3*/
    table icmp_packets{
        key = {}
        actions = {drop;}
        size = 1024;
...
/* table for dropping udp packets when packets are to be transferred between h1 and h2*/
    table udp_packets{
        key = {}
        actions = {drop;}
        size = 1024;
    ...
    apply {
/* applying the tables when any of the below conditions match*/        
        if((((hdr.ipv4.srcAddr == h1) && (hdr.ipv4.dstAddr == h3))||((hdr.ipv4.srcAddr == h3)
             &&(hdr.ipv4.dstAddr == h1))) &&(hdr.ipv4.protocol == 0x01)){
            icmp_packets.apply();
        }
        else if((((hdr.ipv4.srcAddr == h1) && (hdr.ipv4.dstAddr == h2)) 
                  ||((hdr.ipv4.srcAddr == h2)&&(hdr.ipv4.dstAddr == h1)))
                && (hdr.ipv4.protocol == 0x11)){
               udp_packets.apply();
        }
/* it automatically validates ipv4 header ,if it is valid then it will starts match action process*/
